...
if (NumberOfVerses > 9) AND (NumberOfVerses <= 30)
	MsgBox, 262192, Number of Verses more than 10, There are more than 10 verses in >  %Name%  <`n`nYou will need to go into the generated AHK file and manually change "n::" (where n is the verse number above 10) to another key`, up to verse %NumberOfVerses%. `n`nThe new keys can be the letters from QWERTY ----> CVBNM on the keyboard`n`nPlease see Issue #4 in Readme.txt
...
GlossaryMove:
MouseGetPos, x, y, Screen
Tooltip, [F1-F4]     Click at saved position   |     [A and D]   Move Cursor 1px``n[F5-F8]     Save Custom Position   |   [Numpad+]     Toggle Help``n[Pause]        Toggle Save Sound     |   [Numpad *]            Vol+      ``n[X]                       Click Mouse          |   [Numpad - ]            Vol- ``n[Ctrl+Alt+S]     Pause Hotkeys        |      [Ctrl + 1]    Exit Surah Viewer, (x+37), (y-5), 1
return
}
...
Loop ; Generates verse templates up to the number that the user specified
{
	if A_index > %NumberOfVerses%
		...
		SendInput % ("---------------")("{enter}") (Hotkey%A_index%) (":: ")(";<--- Change this number into a key that you would like to use instead")("{enter}{enter}{enter} ")("; | Verse ") A_index (" | ")( ";")("{enter}{enter}{backspace}")("{enter}")("Tooltip")("{enter}{enter}")("MouseClick, left, ") VerseCoords%A_Index% ("{enter}{enter}")("Return")("{enter}{enter}{enter}{enter}{backspace}")
