...
{
	SetTimer, ontop, -50
	InputBox, Link, Surah Viewer - by Turbofan20, What is the Youtube URL of %Name%?,,,,,,locale
	if ErrorLevel
		ExitApp
	else if (Link = "")
	{
		MsgBox Please enter a video URL or click "Don't use URL"
		Goto SurahLink
	}
}
...
{
 SetTimer, OpenGlossaryUponStartup, -50
 NumpadAdd::

SetTimer, GlossaryMove, 1000
...
Else SetTimer, GlossaryMove, 20
GlossaryMove:
MouseGetPos, x, y, Screen
ToolTip , [F1-F4] Click at saved position |         [X] to click mouse``n[F5-F8] Save Custom Position  |   [A and D] to move cursor ``n[Pause] Toggle Hotkey Sound  |   [Numpad+] Toggle Help, (x+37), (y-5), 1
Return
}
